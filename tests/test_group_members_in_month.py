...
def test_night_dedicated_is_unassigned(db):
    """N전담(is_night_nurse=['N'])은 'N전담' 배지 + is_night_dedicated=True.

    236e4cb/99309b0: N전담도 직접 배정된 팀이 있으면 as_of_team 에 노출
    (이전 정책 = 항상 None 폐기). 표시·엑셀 export 일관성 확보.
    """
    db.add(Office(office_id="o1", office_name="병원"))
    db.add(Group(group_id="A", group_name="A병동", office_id="o1"))
    ...
    assert by_id["night"]["as_of_team"] == 2  # 직접 배정 team_id 그대로 노출
...
def test_night_dedicated_robust_string_form(db):
    ...
    236e4cb/99309b0 정책에 따라 as_of_team 은 직접 배정 team_id 노출.
    """
    db.add(Office(office_id="o1", office_name="병원"))
    db.add(Group(group_id="A", group_name="A병동", office_id="o1"))
    ...
    assert by_id["night2"]["as_of_team"] == 1  # 직접 배정 team_id 그대로
