# tests/test_is_night_nurse_write_redirect.py
"""1순위 — is_night_nurse 옛 쓰기 경로를 nurse_allowed_shift_period 로 일원화.
...
def test_agent_update_writes_period_and_projects_cache(seeded):
    db = seeded
    res = update_nurse_attribute(db, "n1", "A", "is_night_nurse", ["N"])
    ...
    assert db.query(Nurse).filter_by(nurse_id="n1").first().is_night_nurse == ["N"]


def test_agent_update_change_is_close_before_open(seeded):
    db = seeded
    update_nurse_attribute(db, "n1", "A", "is_night_nurse", ["N"])
    update_nurse_attribute(db, "n1", "A", "is_night_nurse", ["D", "E"])
    ...
    assert db.query(Nurse).filter_by(nurse_id="n1").first().is_night_nurse == ["D", "E"]
