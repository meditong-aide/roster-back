    ...
    if validation_error:
        ...
        if _trigger_soft and not bool(config_dict.get("_force_grade_max_soft_fallback")):
            ...
            if not retry_validation_error:
                ...
                # 노드 표시 (사용자 정책 2026-05-17): grade_min/max 양쪽 soft 전환을
                # 분리 라벨로 명시. legacy "grade_hard_to_soft" 도 호환 유지.
                # grade_config.allow_soft_fallback 가 grade_min/max 동시 적용이므로
                # 둘 다 풀린 것으로 기록.
                applied_relaxations.append("grade_hard_to_soft")
                applied_relaxations.append("treatment:soft:grade_max")
                applied_relaxations.append("treatment:soft:grade_min")
                weekly_off_warnings.append(
                    {
                        "type": "grade_hard_to_soft_applied",
                        "detail": (
                            "Grade hard 제약(min+max)이 infeasible로 인해 자동 soft 전환되어 "
                            "재생성됐습니다. 일부 grade 최소/최대가 미충족일 수 있습니다."
                        ),
                    }
                )
                print(
