...
#
#   지표 10개 중 off_range 1번이 8개 최선:
#     n2n 380.5→341.0(-10.4%) · 고립OFF 28→24(-14.3%) · N폭 73→68.5 · N편차 18.0→17.1
#     OFF폭 21.5 동률 · 하드 0 동률 · 커버부족 4.5 동률
#   지는 것은 grade 미달 14.5→16.5 뿐인데, **병동별로 보면 방향이 갈린다**:
#     51병동-RN 7.5→7.0 · 42병동-RN 1.0→0.5 (개선)
#     9병동-9A  6.0→9.0 (악화) ← 여기 하나가 합계를 뒤집는다
#   9병동-9A 는 grade≤1 요구 90칸 대 실질 가용 60칸으로 **구조적 공급 부족** 병동이다
#   (진단기 검출). 못 채울 grade 를 붙들지 않고 n2n(-15.5)·고립OFF(-4.5) 로 전환한 셈.
#
#   ★ grade 여유가 있는 병동에서 손해가 없는지 따로 확인했다 —
#     남촌 중환자실1(37명·grade1 5명)은 세 조건 모두 grade 4.0 으로 **완전 동일**.
#     즉 grade 손해는 공급 부족 병동에만 나타나고, 여유가 있으면 순서에 둔감하다.
#
