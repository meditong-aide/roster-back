...
def _config_dict(row: RosterConfig) -> dict:
    return {
        ...
        # config_version 은 dev 에서 RosterConfig 컬럼 아님 → 제외.
        "version": row.version,
        ...
        "ban_night_before_fixed_off": bool(row.ban_night_before_fixed_off) if row.ban_night_before_fixed_off is not None else None,
        ...
        "off_first": bool(row.off_first),
        "off_swap_enabled": bool(row.off_swap_enabled),
    }
