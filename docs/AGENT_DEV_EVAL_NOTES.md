...
> 작성 2026-07 (세션 누적). 평가 하니스는 재실행 가능 — 아래 경로 참조(2026-08-06 경로 3개 실재 확인).
>
> ⚠️ **수치는 측정 시점 스킬셋 기준이다.** 이후 tool 이 22 → 29 로 늘었으므로 §5 의 회귀 게이트
> 원칙대로 라우터 recall·가이드 검색은 재측정 대상이다. 인용 시 "N=..., 커밋 ..., 스킬 N개 시점"을 함께 적을 것.
